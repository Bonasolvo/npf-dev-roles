...
//                itemdblclick : function(){ }
    ...
    onModelListFilter: function(field, newValue, oldValue, eOpts){
        ...
        store.filter([{
            fn: function(record) {
                // Возвращает true если название закладки попадает под условие фильтрации и
    ...
    onModelListItemContextMenu: function(view, record, item, index, e, eOpts) {
        // Создаем контекстное меню для закладки, если оно ещё не создано и
        // показываем в позиции клика мышкой
        e.stopEvent();
